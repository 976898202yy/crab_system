<template>
    <el-dialog title="权限" :visible.sync="visible">
        <el-form :model="dataForm"
            ...
            ref="dataForm"
            label-width="100px">
            <el-form-item label="基地管理员:" prop="userList">
                <el-select
...
import { adjust, findPersion } from '@/api/admin/site'
export default {
    data() {
        return{
            visible: false,
            ...
            dataRule:{
                userList: [
                    { required: true, message: "基地管理员不能为空", trigger: "blur" },
                ],
            },
            role: [],
        }
    ...
    methods:{
        init(row){
            this.dataForm.siteId = row.siteId;
            this.visible = true;
            this.loadData();
            findPersion(row.siteId).then(resoult => {
        ...
        loadData(){
            getPermission('基地管理员').then(resoult => {
                let res = resoult.data;
                if(res.code == 0){
                    this.role = res.data;
                }
