...
<script>
import { fetchList, addObj, putObj, delObj, getCascader, getEnterprise } from "@/api/admin/site";
import {tableOption} from '@/const/crud/admin/site';
...
export default {
  ...
  methods: {
    ...
    deletes(row, index) {
      this.$confirm(
        "此操作将永久删除该基地(基地名称:" + row.name + "), 是否继续?",
        "提示",
        {
    ...
    //  导出excel
    exportExcel() {
      this.downBlobFile("/admin/site/importExcel", this.searchForm, "基地表.xlsx");
    },
  },
};
</script>
